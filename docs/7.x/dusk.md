# Royalcms 测试之 - 浏览器测试 (Royalcms Dusk)
...
> {info} 永远不要在生产环境安装 Dusk。否则，任何人都可以未经授权地访问你的应用。
...
> {primary} 这个测试用例可以测试由 `make:auth` royalcms 命令来生成的登录页面。
...
> {info} 使用 `loginAs` 方法后, 该用户的 session 将会持久化供其他测试用例使用。
...
> {info} 这方法基于 JQuery 来进行交互。如果页面中没有可用的 jQuery，Dusk 会自动将 jQuery 注入到页面中。所以他可能会增加测试的时间。
...
> {primary} 所有包在 `{}` 中的修饰按键，都应该与 `Facebook\WebDriver\WebDriverKeys` 类中定义的常量一致。你可以在 [GitHub 中找到这个类](https://github.com/facebook/php-webdriver/blob/community/lib/WebDriverKeys.php)。
