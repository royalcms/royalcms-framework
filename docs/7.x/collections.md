...
> {primary} 默认情况下， [Eloquent](/docs/{{version}}/eloquent) 查询的结果返回的内容都是 `Collection` 实例。
...
> {info} 与大多数集合的方法不同，`forget` 不会返回修改过后的新集合；它会直接修改原来的集合。
...
> {info} 像其他集合方法一样，`map` 返回一个新的集合实例；它不会修改它所调用的集合。如果你想改变原集合，得使用 [`transform`](#method-transform) 方法。
...
> {primary} 如果要对嵌套数组或对象的集合进行排序，参考 [`sortBy`](#method-sortby) 和 [`sortByDesc`](#method-sortbydesc) 方法。
...
> {info} `toArray` 也会将所有集合的嵌套对象转换为数组。如果你想获取原数组，就改用 [`all`](#method-all) 方法。
...
> {info} 与大多数集合的方法不同，`transform` 会修改集合本身。如果你想创建新的集合，就改用 [`map`](#method-map) 方法。
